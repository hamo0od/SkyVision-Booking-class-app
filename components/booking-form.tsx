...
export function BookingForm({ classrooms, onBookingSuccess }: BookingFormProps) {
  ...
  useEffect(() => {
    if (showSuccessMessage && successRef.current) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Call the callback if provided (to refresh booking list)
        if (onBookingSuccess) {
          onBookingSuccess()
        }
  ...
  return (
    <div ref={formRef}>
      {/* Success Message */}
      {showSuccessMessage && (
        <div ref={successRef} className="mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-green-600 mt-2">
                    Your booking request has been sent to the admin for approval. You will be notified once it's
                    reviewed.
                  </p>
                </div>
              </div>
