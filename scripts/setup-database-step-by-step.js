...
async function main() {
  ...
  try {
    ...
    // Step 3: Create enum types first
    console.log("3. Creating enum types...")
    await createEnumTypes()

    // Step 4: Create basic tables
    console.log("4. Creating basic tables...")
    await createBasicTables()

    // Step 5: Create the new separated tables
    console.log("5. Creating separated Materials and Tools tables...")
    ...
    // Step 8: Seed with sample data
    console.log("8. Seeding database with sample data...")
    await seedDatabase()

    // Step 9: Generate Prisma client
    console.log("9. Generating Prisma client...")
    const { exec } = require("child_process")
    await new Promise((resolve, reject) => {
      exec("npx prisma generate", (error, stdout, stderr) => {
        if (error) {
          console.log("Note: Please run 'npx prisma generate' manually")
          resolve()
        } else {
    ...
    console.log("   2. Run 'npx prisma generate' if not done automatically")
...
async function resetDatabase() {
  try {
    ...
    console.log("Database reset completed with warnings (this is normal)")
  }
}
...
async function createEnumTypes() {
  try {
    ...
    // Create CheckoutPriority enum type
    await prisma.$executeRaw`
      DO $$
      BEGIN
    ...
    // Create MaterialStatus enum type
    await prisma.$executeRaw`
      DO $$
      BEGIN
...
async function createSeparatedTables() {
  ...
  console.log("✅ Separated tables created")
}
